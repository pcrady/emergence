...
class Particle(object):
    ...
    def __leapfrog_position(self) -> None:
        self.position_vector = (self.position_vector + (self.velocity_vector * self.time_delta) + (0.5 * self.force_vector * self.time_delta**2)).astype(np.float32)
        self.__handle_walls()

    def __leapfrog_velocity(self) -> None:
        self.velocity_vector = (self.velocity_vector + 0.5 * (self.previous_force_vector + self.force_vector) * self.time_delta).astype(np.float32)
        self.__handle_max_velocity()
